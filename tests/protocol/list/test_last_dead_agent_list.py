...
def test_set_received_info(initialize_json:dict, last_dead_agent_list_json:dict) -> None:
    test_list = LastDeadAgentList.initialize_from_json(set_list=initialize_json["gameInfo"]["lastDeadAgentList"])

    assert test_list == []

    test_list = LastDeadAgentList.initialize_from_json(set_list=last_dead_agent_list_json["gameInfo"]["lastDeadAgentList"])
