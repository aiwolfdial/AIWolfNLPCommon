...
def test_info(initialize_json):
    ...
    assert setting.role_num_map.get_role_num(role=RoleInfo.VILLAGER) == initialize_json[setting_key]["roleNumMap"]["VILLAGER"]
    assert setting.role_num_map.get_role_num(role=RoleInfo.VILLAGER.value) == initialize_json[setting_key]["roleNumMap"]["VILLAGER"]
